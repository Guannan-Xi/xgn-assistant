...
class UnifiedWorkbench(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(side, text="任务参数在这里；模型、Key、邮箱请到网页总控台统一配置。", style="SideBody.TLabel", wraplength=190).pack(anchor="w", pady=(8, 20))
    ...
    def _build_model_page(self) -> None:
        ...
        ttk.Label(self.model_page, text="Key 文件", style="PanelTitle.TLabel").grid(row=18, column=0, columnspan=3, sticky="w", pady=(16, 8))
        row = 19
    ...
    def _build_centralized_config_page(self, parent: ttk.Frame, title: str, body: str, fragment: str) -> None:
        ...
        ttk.Button(actions, text="打开总控台", style="Primary.TButton", command=lambda: self._open_total_console(fragment)).pack(side="left")
        ttk.Button(actions, text="打开工具页", style="Tool.TButton", command=lambda: self._open_total_console("#more")).pack(side="left", padx=(10, 0))
        ttk.Label(
            parent,
            text="这个旧桌面入口仍会在任务启动时读取总控台保存的本地配置，但不再提供模型、Key、SMTP 的编辑入口。",
            style="Subtle.TLabel",
            wraplength=760,
    ...
    def _open_advanced_config(self) -> None:
        ...
        self._build_centralized_config_page(
            self.model_page,
            "模型与 Key 由总控台统一管理",
            "模型 URL、模型名、方案切换和所有大模型 Key 已集中到网页总控台。旧桌面入口不再保存或测试模型配置。",
            "#model",
        )
        self._build_centralized_config_page(
            self.email_page,
            "邮箱与 SMTP 由总控台统一管理",
            "收件邮箱、SMTP 服务器、账号和授权码已集中到网页总控台。旧桌面入口不再保存或测试邮箱配置。",
            "#more",
        )
        notebook.add(self.task_settings_page, text="任务参数")
        notebook.add(self.model_page, text="模型/Key（总控台）")
        notebook.add(self.email_page, text="邮箱/SMTP（总控台）")
    ...
    def _render_science_classic(self) -> None:
        ...
        ttk.Label(self.task_page, text="流程开关放在左侧“高级配置”；模型、Key 和邮箱在网页总控台统一配置。", style="Subtle.TLabel").grid(row=4, column=1, sticky="w", pady=(8, 0))
    ...
    def _render_culture(self) -> None:
        ...
        ttk.Label(self.task_page, text="起始阶段、复用、生图和后处理放在左侧“高级配置”；模型、Key 和邮箱在网页总控台统一配置。", style="Subtle.TLabel").grid(row=5, column=1, sticky="w", pady=(8, 0))
    ...
    def _normalized_model_base_url(self, var: tk.StringVar) -> str:
        base_url = (var.get().strip() or self._normalized_foreign_base_url()).replace("greatwallink.top", "greatwalllink.top").rstrip("/")
        if not base_url.endswith("/v1"):
            base_url += "/v1"
        return base_url
